        ...
        try {
            $wallet = Wallet::join('coins', 'coins.id', '=', 'wallets.coin_id')
                ...
                ->select('wallets.*', 'coins.status as coin_status', 'coins.is_withdrawal', 'coins.minimum_withdrawal',
                    'coins.maximum_withdrawal', 'coins.withdrawal_fees','coins.coin_icon')
                ->first();
            if (!empty($wallet)) {
        ...
        try {
            ...
            if (isset($exists) && (!empty($exists->address))) {
                ...
                if (!empty($address['address'])) {
                    ...
                    $history->AddWalletAddressHistory($data['wallet']->id, $address['address'], $data['wallet']->coin_type,$address['wallet_key']);
                }
            }

            $data['wallet']->network = api_settings($data['wallet']->network);
