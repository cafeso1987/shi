...
class WalletController extends Controller
{
    ...
    public function walletHistoryApp(Request $request){
        ...
        $response = [];
        $data = [];
        if(isset($request->type) && ($request->type == 'deposit' || $request->type == 'withdrawal')) {
    ...
    public function coinSwapHistoryApp(Request $request){
        ...
        $data['list'] = WalletSwapHistory::where(['user_id' => Auth::id()])->orderBy($order_data['column_name'], $order_data['order_by'])->paginate($limit);
     ...
     * get rate of coin
     */
    public function getRateApp(CoinSwapRequest $request)
    {
