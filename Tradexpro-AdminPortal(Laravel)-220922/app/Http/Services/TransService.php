    ...
    public function acceptPendingExternalWithdrawal($transaction)
    {
        try {
            ...
            if ($coin->network == COIN_PAYMENT) {
                ...
                Log::info('acceptPendingExternalWithdrawal : coin payment');
                $result = $this->external_transfer_using_coin_api($transaction->coin_type,$transaction->address, $transaction->amount, Auth::id(), true, $transaction->user_id);
    ...
    public function external_transfer_using_coin_api($coinType,$address, $amount, $authId, $isAdmin, $user_id)
    {
        try {
            $coin = Coin::join('coin_settings','coin_settings.coin_id', '=', 'coins.id')
                ...
                ->first();
            if ($coin) {
                $api =  new BitCoinApiService($coin->coin_api_user,$coin->coin_api_pass,$coin->coin_api_host,$coin->coin_api_port);
                    ...
                    if ($checkKyc['success'] == false) {
                        ...
                        if (!empty($request->code)) {
                            ...
                            if ($valid) {
                                ...
                                $response = [
                                    'success' => false,
                                    'message' => __('Verify code is required'),
                                    'data' => ''
                                ];
                            }
                        } else {
    ...
    public function sendCoinWithBitgo($transaction)
    {
        try {
            $coin = Coin::join('coin_settings','coin_settings.coin_id', '=', 'coins.id')
                ...
                ->first();
            if ($coin) {
                $response = $this->sendBitgoCoin($coin->coin_type,$coin->bitgo_wallet_id,$transaction->amount,$transaction->address,$coin->bitgo_wallet);
            } else {
                $response = [
                    'success' => false,
